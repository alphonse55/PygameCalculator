        ...
        if event.type == pygame.QUIT:
            ...
            for button in config.buttons:
                if button.mouse_on_button(pos):
                    ...
                    break
    try: pygame.draw.rect(screen, config.BLACK, operation_rect)
    except NameError: pass

    try:
        try: pygame.draw.rect(screen, config.BLACK, last_operation_rect)
        except NameError: pass
        last_operation_render = config.font_50.render(last_operation, True, config.WHITE)
        last_operation_rect = last_operation_render.get_rect(midleft = (config.SIDE_LENGTH/3, config.SIDE_LENGTH/2))
        screen.blit(last_operation_render, last_operation_rect)
    except NameError: pass

    operation_render = config.font_70.render(operation, True, config.WHITE)
